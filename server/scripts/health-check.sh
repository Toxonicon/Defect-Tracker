...
print_header() {
    echo -e "\n${BLUE}========================================${NC}"
    echo -e "${BLUE}  Defect-Traker Health Check${NC}"
    echo -e "${BLUE}========================================${NC}\n"
}
...
if pm2 list | grep -q "defect-traker-server"; then
...
ERROR_COUNT=$(pm2 logs defect-traker-server --lines 100 --nostream 2>/dev/null | grep -i "error" | wc -l)
